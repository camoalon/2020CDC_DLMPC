...
mu = 50; % 50 n = 5 t = 1.5059 obj-obj_VAL = 0.0290
...
%                 cvx_begin quiet
%                 variable W_cvx(n+m-1,1)
%                 minimize(W_cvx'*((C_proxi*M2)'*(C_proxi*M2)+rho/2*(M1'*M1)+mu/2*Mj_sum)*W_cvx+(-rho*a*M1 -mu*Mjb_sum')*W_cvx)
%                 subject to
%                 K_proxi*M2*W_cvx <= up;
%                 cvx_end
%                
%                 W_VAL = pinv(2*((C_proxi*M2)'*C_proxi*M2)+rho*(M1'*M1)+mu*Mj_sum)*(rho*M1'*a'+mu*Mjb_sum);
...
% cvx_begin
% variable Phi_VAL(Nx*T+Nu*(T-1),Nx)
% 
% vect1 = vec(C*Phi_VAL*xi);
% minimize(vect1'*vect1);
% 
% [K zeros(2*Nx*T,Nu*(T-1))]*Phi_VAL*xi <= up*ones(2*Nx*T,1);
% IZA_ZB*Phi_VAL == E1;
% 
% for sys = 1:Nx
%     for i = r{sys}
%         indeces_zero = setdiff(1:Nx,s_r{sys}(find(r{sys}==i),find(s_r{sys}(find(r{sys}==i),:))));
%         Phi_VAL(i,indeces_zero) == zeros(1,max(length(indeces_zero)));
%     end
% end
% cvx_end
% 
% norm(C*Phi_VAL*xi,'fro') - norm(C*VAL*xi,'fro')
