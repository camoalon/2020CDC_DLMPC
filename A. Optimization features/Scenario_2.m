...
%     sys = 1; % Separate since we want to measure time %STUDY COUPLING HERE
%     tic;
%     for j = r{sys}
%         Z_admm{j} = 0;
%         for i = [find(C(:,j))]'
...
% vect2 = vec(Phi_VAL-Psi+Lambda);
% 
% minimize(vect1'*vect1+rho/2*(vect2'*vect2))
% 
% for sys = 1:Nx
%     for i = r{sys}
...
% 
% vect2 = vec(Phi-Psi+Lambda);
% 
% minimize(rho/2*(vect2'*vect2))
% 
% IZA_ZB*Psi == E1;
% 
% for sys = 1:Nx
%     for i = r{sys}
%         indeces_zero = setdiff(1:Nx,s_r{sys}(find(r{sys}==i),find(s_r{sys}(find(r{sys}==i),:))));
%         Psi(i,indeces_zero) == zeros(1,max(length(indeces_zero))); 
%     end
% end
% 
% cvx_end
